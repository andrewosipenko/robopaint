...
public abstract class AbstractAreaTransformer implements LineImageTransformer {
    ...
    protected int getArea(IndexedLineImage source) {
        Random random = ThreadLocalRandom.current();
        int maxAreaSize = (int)(ratio * (source.getLineCount() - minAreaSize));
    ...
    protected int getStart(IndexedLineImage source, int area) {
        int maxStart = source.getLineCount() - area;
        if (maxStart == 0) {
            return 0;
        }
        Random random = ThreadLocalRandom.current();
